...
structure = load('dataHyperMeta.mat');
allCenters = []; 
...
for i = 1:size(structure.mask,3)
    ...
    imgEdge = edge(imgNew{i,1},'canny', 0.3);
    [centers, radii, metric]=imfindcircles(imgEdge,[6 15], 'Sensitivity', 0.9);
...
for i = 1: velikost(1)
    ...
    rozdilSouradnic
    if rozdilSouradnic < 12   % Hyper 12
