...
structure = load('dataCombMeta.mat');
allCenters = []; 
...
for i = 1:size(structure.mask,3)
    ...
    imgEdge = edge(imgNew{i,1},'canny', 0.17);
    [centers, radii, metric]=imfindcircles(imgEdge,[5 18], 'Sensitivity', 0.88);
     
   
    %figure(i+35); imshow(imgEdge);
    %figure(1); subplot(6,7,i); imshow(imgEdge);
...
% Získání stredù
[ids, ctrs] =  kmeans(allCenters(:,1:2),3,'replicates', 100);
...
for i = 1: velikost(1)
    ...
    if rozdilSouradnic < 13   % Hyper 12
